...
export default function AllEntries({ isDarkMode }: SettingsProperties){
    ...
    return(
        <section className="grid grid-cols-2 md:grid-cols-4">
            {entries.map((entry: Entry, index: number) => {
                return(
                    <div id={entry.id} key={index}className={`${isDarkMode ? 'bg-gray-700 text-white' : 'bg-gray-300 text-black'} shadow-md shadow-gray-500 m-3 p-4 rounded flex flex-col justify-between`}>

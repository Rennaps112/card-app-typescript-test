...
server.post<{ Body: Entry }>("/create/", async (req, reply) => {
  ...
  newEntryBody.deadline ? (newEntryBody.deadline = new Date(req.body.deadline)) : (newEntryBody.deadline = new Date());
  try {
...
server.put<{ Params: { id: string }; Body: Entry }>("/update/:id", async (req, reply) => {
  ...
  updatedEntryBody.deadline
    ? (updatedEntryBody.deadline = new Date(req.body.deadline))
    : (updatedEntryBody.deadline = new Date());
  try {
